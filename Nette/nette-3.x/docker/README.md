...
```bash
docker-compose -f ./nette-3.x/docker/docker-compose.volume.yaml -p nette3-volume up -d --build
```
...
```bash
docker-compose -f ./nette-3.x/docker/docker-compose.yaml -p nette3 up -d --build
```
...
```bash
docker build -f ./nette-3.x/docker/Dockerfile -t nette3:run /nette-3.x/docker
```
...
或者使用镜像模式产生镜像：（`nette3:latest`），具体请查看`docker-compose.yaml`。

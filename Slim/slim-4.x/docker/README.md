...
```bash
docker-compose -f ./slim-4.x/docker/docker-compose.volume.yaml -p slim4-volume up -d --build
```
...
```bash
docker-compose -f ./slim-4.x/docker/docker-compose.yaml -p slim4 up -d --build
```
...
```bash
docker build -f ./slim-4.x/docker/Dockerfile -t slim4:run /slim-4.x/docker
```
...
或者使用镜像模式产生镜像：（`slim4:latest`），具体请查看`docker-compose.yaml`。

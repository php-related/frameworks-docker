...
```bash
docker-compose -f ./yii-1.1/docker/docker-compose.volume.yaml -p yii1_1-volume up -d --build
```
...
```bash
docker-compose -f ./yii-1.1/docker/docker-compose.yaml -p yii1_1 up -d --build
```
...
```bash
docker build -f ./yii-1.1/docker/Dockerfile -t yii1_1:run /yii-1.1/docker
```
...
或者使用镜像模式产生镜像：（`yii1_1:latest`），具体请查看`docker-compose.yaml`。

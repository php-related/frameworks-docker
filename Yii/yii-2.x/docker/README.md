...
```bash
docker-compose -f ./yii-2.x/docker/docker-compose.volume.yaml -p yii2-volume up -d --build
```
...
```bash
docker-compose -f ./yii-2.x/docker/docker-compose.yaml -p yii2 up -d --build
```
...
docker build -f ./yii-2.x/docker/Dockerfile -t yii2:run /yii-2.x/docker
```
...
或者使用镜像模式产生镜像：（`yii2:latest`），具体请查看`docker-compose.yaml`。

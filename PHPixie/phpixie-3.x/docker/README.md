...
```bash
docker-compose -f ./phpixie-3.x/docker/docker-compose.volume.yaml -p phpixie3-volume up -d --build
```
...
```bash
docker-compose -f ./phpixie-3.x/docker/docker-compose.yaml -p phpixie3 up -d --build
```
...
```bash
docker build -f ./phpixie-3.x/docker/Dockerfile -t phpixie3:run /phpixie-3.x/docker
```
...
或者使用镜像模式产生镜像：（`phpixie3:latest`），具体请查看`docker-compose.yaml`。

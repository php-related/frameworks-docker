...
```bash
docker-compose -f ./laravel-8.x/docker/docker-compose.volume.yaml -p laravel8-volume up -d --build
```
...
```bash
docker-compose -f ./laravel-8.x/docker/docker-compose.yaml -p laravel8 up -d --build
```
...
```bash
docker build -f ./laravel-8.x/docker/Dockerfile -t laravel8:run /laravel-8.x/docker
```
...
或者使用镜像模式产生镜像：（`laravel8:latest`），具体请查看`docker-compose.yaml`。

```bash
docker run -d --name laravel8-latest -p 8070:80 laravel8:latest
```

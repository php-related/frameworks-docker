...
```bash
docker-compose -f ./laravel-10.x/docker/docker-compose.volume.yaml -p laravel10-volume up -d --build
```
...
```bash
docker-compose -f ./laravel-10.x/docker/docker-compose.yaml -p laravel10 up -d --build
```
...
```bash
docker build -f ./laravel-10.x/docker/Dockerfile -t laravel10:run /laravel-10.x/docker
```
...
或者使用镜像模式产生镜像：（`laravel10:latest`），具体请查看`docker-compose.yaml`。

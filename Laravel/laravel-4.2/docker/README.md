# Laravel 4.2 Framework 项目使用说明

本项目基于 **PHP Laravel 4.2** 框架，本项目提供完整源码及两种主流的部署方式，适合不同场景下的开发与部署需求。
...
```bash
docker run -d --name laravel4_2-run -p 8064:80 laravel4_2:run
```

或者使用镜像模式产生镜像：（`laravel4_2:latest`），具体请查看`docker-compose.yaml`。

```bash
docker run -d --name laravel4_2-latest -p 8064:80 laravel4_2:latest
```
...
- nginx 服务器
- MySQL 或其他数据库服务
- 项目源码放置目录，例如 `/var/www/laravel-4.2`

### 2. nginx 配置

项目提供 `nginx.conf` 示例配置(请参考laravel-4.2根目录下nginx.conf)
...
```bash
sudo chown -R www-data:www-data /var/www/laravel-4.2
sudo find /var/www/laravel-4.2 -type f -exec chmod 644 {} \;
sudo find /var/www/laravel-4.2 -type d -exec chmod 755 {} \;
```

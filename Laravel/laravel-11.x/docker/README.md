...
启动容器：
```bash
docker-compose -f /laravel-11.x/docker/docker-compose.volume.yaml -p laravel11-volume up -d --build
```
...
启动容器：
```bash
docker-compose -f /laravel-11.x/docker/docker-compose.yaml -p laravel11 up -d --build
```
...
构建镜像：
```bash
docker build -f /laravel-11.x/docker/Dockerfile -t laravel11:run /laravel-11.x/docker
```
...
或者使用镜像模式产生镜像：（`laravel11:latest`），具体请查看`docker-compose.yaml`。
